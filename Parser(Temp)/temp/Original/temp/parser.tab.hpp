  ...
  private:
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_SimpleDeclaration = 11,                // SimpleDeclaration
        S_VariableDeclaration = 12,              // VariableDeclaration
        S_type = 13,                             // type
        S_PrimitiveType = 14                     // PrimitiveType
      };
    };
...
switch (yykind)
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
      ...
      {
        YY_ASSERT (tok == token::YYEOF || tok == token::YYerror || tok == token::YYUNDEF || tok == token::VAR || tok == token::COL || tok == token::ITYPE || tok == token::EOL);
      }
#else
      symbol_type (int tok)
        : super_type(token_type (tok))
