...
#line 76 "parser.ypp"
                                             { printf("defined variable \"%s\" with type %s\n", yystack_[2].value.as < std::string > ().c_str(), yystack_[0].value.as < std::string > ().c_str()); }
...
#line 106 "parser.ypp"
                                              {   yylhs.value.as < std::string > () = "record"; printf("record was defined\n"); }
#line 790 "parser.tab.cpp"
    break;

  case 25: // ArrayType: ARY LAR Expression RAR type
#line 109 "parser.ypp"
                                       {  yylhs.value.as < std::string > () = "array"; printf("array was defined\n"); }
...
#line 156 "parser.ypp"
                                      { yylhs.value.as < std::string > () = yystack_[2].value.as < std::string > () + "+"   + yystack_[0].value.as < std::string > (); printf("%s+%s\n", yystack_[2].value.as < std::string > ().c_str(), yystack_[0].value.as < std::string > ().c_str()); }
...
#line 167 "parser.ypp"
                            {           yylhs.value.as < std::string > () = yystack_[2].value.as < std::string > () + "and" + yystack_[0].value.as < std::string > ();  printf("%s and %s\n", yystack_[2].value.as < std::string > ().c_str(), yystack_[0].value.as < std::string > ().c_str()); }
#line 898 "parser.tab.cpp"
    break;

  case 62: // Expression: Expression OR Expression
#line 168 "parser.ypp"
                           {            yylhs.value.as < std::string > () = yystack_[2].value.as < std::string > () + "or"  + yystack_[0].value.as < std::string > (); printf("%s or %s\n", yystack_[2].value.as < std::string > ().c_str(), yystack_[0].value.as < std::string > ().c_str()); }
#line 904 "parser.tab.cpp"
    break;

  case 63: // Expression: Expression XOR Expression
#line 169 "parser.ypp"
                            {           yylhs.value.as < std::string > () = yystack_[2].value.as < std::string > () + "xor" + yystack_[0].value.as < std::string > ();   printf("%s xor %s\n", yystack_[2].value.as < std::string > ().c_str(), yystack_[0].value.as < std::string > ().c_str()); }
#line 910 "parser.tab.cpp"
    break;

  case 64: // Expression: SUB Expression
#line 170 "parser.ypp"
                 { yylhs.value.as < std::string > () = yystack_[0].value.as < std::string > ();  printf("unary minus\n"); }
...
#line 172 "parser.ypp"
                     { yylhs.value.as < std::string > () = yystack_[1].value.as < std::string > (); printf("braces around %s\n", yystack_[1].value.as < std::string > ().c_str()); }
...
#line 974 "parser.tab.cpp"
  ...
  parser::yyrline_[] =
  {
       0,    66,    66,    67,    68,    69,    72,    73,    76,    77,
      78,    81,    84,    85,    88,    89,    92,    95,    96,    97,
      98,   101,   102,   103,   106,   109,   110,   113,   114,   115,
     116,   119,   120,   121,   122,   123,   124,   127,   128,   131,
     134,   137,   140,   143,   144,   147,   148,   151,   152,   153,
     156,   157,   158,   159,   160,   161,   162,   163,   164,   165,
     166,   167,   168,   169,   170,   171,   172,   175,   176,   179,
     180,   181,   182,   183,   184,   187,   188,   189,   192
  };
